...
## Low-risk closure after Linux delivery

The report-cache capacity eviction path now cancels affected scene participation just like TTL expiry, and enforces the bound within the inserting report call. A failing-before/passing-after regression and a fresh-coordinator no-replay regression bring root tests to **420/420**; Clawd/renderer suites remain green and their source/pins are unchanged. See the [night review](GATHERING-NIGHT-REVIEW.md).

Next operator entry point: [Windows CMD update and acceptance checklist](GATHERING-WINDOWS-ACCEPTANCE.md). Windows-specific gates and long-bubble visual work remain pending; no FIFO or new scene feature was added in this closure.
