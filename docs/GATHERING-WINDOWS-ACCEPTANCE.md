...
状态：**Windows 单屏 PoC 已获用户确认并收口；呈现修复看起来正常、区域重启持久化正常，多屏/混合 DPI 未测。** 下方保留完整操作清单；本段只归档实际反馈，不把未逐项报告的条目一并勾通过。
...
- 后续反馈：呈现hotfix用户表示看起来正常、细节先这样；区域“重启回默认”实为此前Apply冲突测试改回原位，重新重启测试正常。
- 后续路线以 [PLAN 当前执行重点](../PLAN.md) 为准：优先多session桌宠体验与H3小样实验，FIFO不再是自动下一步。当前没有已知阻断下一步的单屏缺陷；自动Gather仍未启用。

本次只验活动区域和显式 Gathering；不加 FIFO、自动漫游、Team 权限或模型编排。完整边界见 [Gathering design](GATHERING-SCENE-DESIGN.md)。

## 呈现 hotfix：用户反馈收口，保留细分回归清单
...
用户随后反馈“看起来是ok了，细节先这样”，据此收口当前单屏PoC。以上细分条目保留为以后回归清单，不伪造逐项勾选；Linux VM/Chromium/WebKitGTK通过不替代Windows特定行为验证。多屏/混合DPI与未逐项反馈的高级焦点/模态边界仍保持未测，不阻塞当前PoC下一切片。
