...
- Bounded peer messaging, receiver-local opt-in wake, autonomous lite Teams with leader-only online add and offline-safe remove, and a Team-scoped revisioned Markdown Board with OCC.
- Session-persistent user permission switches: wake, Team autonomy and Board write restore from the current Pi branch on resume/reload; new sessions default off. See [persistence semantics](packages/pi-extension/README.md#session-permission-persistence).
- User-defined desktop activity area: pet Settings opens a temporary resizeable frame with Apply/Cancel/Disable and shared coordinator persistence. Explicit Team gather/end now uses that area, yields to manual drag, and leaves pets in place on end. Linux implementation/tests and the reported Windows single-display functional cases are verified; visual review and multi-display/mixed-DPI coverage remain outstanding. See [Gathering design and smoke checklist](docs/GATHERING-SCENE-DESIGN.md).
